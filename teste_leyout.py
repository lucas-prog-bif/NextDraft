    ...
    if st.button("💾 Salvar Atributos no Banco", use_container_width=True):
        try:
            ...
            if existe:
                ...
                cursor.execute("INSERT IGNORE INTO atletas (id_atleta, nome, posicao_principal, cidade, estado) VALUES (%s, %s, 'Meia', 'São Paulo', 'SP')", (id_usuario_atual, nome_usuario_atual))
        ...
        if st.button("Cadastrar e Entrar", use_container_width=True):
            if len(novo_nome) < 3:
                ...
                if cursor.fetchone():
                    ...
                    try:
                        cursor.execute(
                            "INSERT INTO usuarios ( tipo_perfil, username, email, senha) VALUES (%s, %s, %s, %s)", 
                            (tipo_perfil, novo_nome, novo_email, nova_senha)
                        )
                        conn.commit()
